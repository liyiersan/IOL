...
'''
    RepLKNet for ImageNet-1K, implemented in PyTorch.
...
def get_conv2d(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias):
    ...
    # No need to add depthwise_conv2d_implicit_gemm support here
    # as it is only speeds up training with large batch size
    # details refer to https://github.com/DingXiaoH/RepLKNet-pytorch/issues/17
...
def create_RepLKNet31B(drop_path_rate=0.3, num_classes=1000, use_checkpoint=True, small_kernel_merged=False):
    return RepLKNet(large_kernel_sizes=[31,29,27,13], layers=[2,2,18,2], channels=[128,256,512,1024],
                    drop_path_rate=drop_path_rate, small_kernel=5, num_classes=num_classes, out_indices= [0,1,2,3], use_checkpoint=use_checkpoint,
...
def create_RepLKNetXL(drop_path_rate=0.3, num_classes=1000, use_checkpoint=True, small_kernel_merged=False):
    return RepLKNet(large_kernel_sizes=[27,27,27,13], layers=[2,2,18,2], channels=[256,512,1024,2048],
                    ...
                    num_classes=num_classes, out_indices= [0,1,2,3], use_checkpoint=use_checkpoint,
                    small_kernel_merged=small_kernel_merged)

if __name__ == '__main__':
    model = create_RepLKNet31B(small_kernel_merged=False).cuda()
    model.eval()
    print('------------------- training-time model -------------')
    print(model)
    # model.out_indices = [0, 1, 2, 3]
